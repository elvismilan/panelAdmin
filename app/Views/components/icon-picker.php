 ...
 * Variables esperadas:
 *   $iconActual   string  — valor actual (clase FA, puede ser vacío)
 *   $iconOptions  array   — resultado de IconHelper::options() o optionsWithSelected()
 *   $pickerId     string  — ID único para este picker en la página (default: 'ele_icono')
 */
...
<div id="grupo_icono">
    ...
    <input type="hidden" id="<?= htmlspecialchars($_pickerId, ENT_QUOTES, 'UTF-8') ?>"
           name="ele_icono"
           value="<?= htmlspecialchars($_current, ENT_QUOTES, 'UTF-8') ?>">

    <div class="input-group input-group-sm mb-2">
        <span class="input-group-text"><i class="fa fa-search"></i></span>
        <input type="text" id="<?= htmlspecialchars($_searchId, ENT_QUOTES, 'UTF-8') ?>"
               class="form-control"
    ...
    <div id="<?= htmlspecialchars($_gridId, ENT_QUOTES, 'UTF-8') ?>"
    ...
    <div class="form-text mt-1">
        Seleccionado:
        <i id="<?= htmlspecialchars($_previewId, ENT_QUOTES, 'UTF-8') ?>"
           class="<?= htmlspecialchars($_current !== '' ? $_current : 'fa fa-ban', ENT_QUOTES, 'UTF-8') ?> ms-1"></i>
